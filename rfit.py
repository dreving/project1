class rfit(object):
    def __init__(self,k,xParamLength,spiltData,fold):
        self.k = k
    ...
    def fill(self)
    	...
    	for f in range(self.fold):
            ...
            traindata = np.concatenate(
                splitdata[np.arange(len(self.splitdata)) != f], axis=1)
	    	p0 = np.ones(self.k)
			...
			if sse < self.sseMin:  # most generalizeable
			    ...
			    self.adr2 = 1 - ((1 - self.r2) * (self.N - 1) / self.N - self.k - 1))
			    self.pbest = p
			return self.pbest
	...
	def getF(self,prevSSE):
		self.F = (prevSSE - self.sseMax) / (self.sseMax / (self.N - self.k))
		return self.F
